        ...
        if isValidEmail {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(2), execute: {
                ...
                if BaseViewModel.shared.isNetworkAvailable {
                    ...
                    self.callForgotPasswordAPI(credentialsManager: credentialsManager)
...
extension ForgotPasswordViewModel {
    func callForgotPasswordAPI(credentialsManager : CredentialsManager) {
        authenticaionAPI.forgotPassword(credentials: credentialsManager) { (forgotPasswordResponse, error) in
            if error == nil {
                self.delegate?.onForgotPasswordResponse(response: forgotPasswordResponse)
            } else {
