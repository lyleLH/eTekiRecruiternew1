...
class PreferencesViewTest: XCTestCase {
    ...
    func testdropdownSetup(){
        ...
        XCTAssertEqual(createJobController.optionalSkillsDropDown.bottomOffset, CGPoint(x: 0, y: 10.0))
        ...
        XCTAssertEqual(createJobController.requiredSkillsDropDown.bottomOffset, CGPoint(x: 0, y: 10.0))
    }
...
//        XCTAssertEqual(createJobController.interviewLocationField.text, "India")
    ...
    func testSegue(){
        let segue = UIStoryboardSegue(identifier: StoryboardSegueIdentifiers.createJobToLangaugeSearch, source: CreateJobViewController(), destination: UINavigationController())
       ...
       createJobController.prepare(for: segue, sender: nil)
        ...
        let segue1 = UIStoryboardSegue(identifier: StoryboardSegueIdentifiers.createJobToAreaExpertise, source: CreateJobViewController(), destination: UINavigationController())
        ...
        let segue2 = UIStoryboardSegue(identifier: StoryboardSegueIdentifiers.createJobToCountrySearch, source: CreateJobViewController(), destination: UINavigationController())
        let vc2 = segue2.destination as? UINavigationController
        let nav2 = vc2!.viewControllers.first as? SearchViewController
        nav2?.currentSearch = StoryboardSegueIdentifiers.createJobToCountrySearch
        createJobController.prepare(for: segue2, sender: nil)
    }
